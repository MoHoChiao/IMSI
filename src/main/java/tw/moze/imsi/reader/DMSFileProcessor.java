	...
	public void run() {
		...
		try {
			...
			if (lastSize == null) { // 第一次處理, 不真的處理，只寫檔案大小
				...
				XXX.out("process " + absSrcPath);
				fileSizeMap.remove(absSrcPath);

				// 2018-09-10 加上處理 DMS 錯誤重複執行的邏輯，大部分可能的原因是 redis server 無法連線
				boolean done = false;
                for (int i = 0; i < 3; i++) {
                    try {
                        readCSV();
				...
				long fileTime = ReaderUtil.getDMSFileTime(absSrcPath);
				ReaderUtil.setMaxDataTime(fileTime);
			}
		}
		finally {
			latch.countDown();
